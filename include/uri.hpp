...
private:
  ...
  void setup(std::string const &uri_text, scheme_category category)
  {
    ...
    if(*cursor == ':')
        cursor++;
    cursor = parse_content(uri_text, cursor);
...
//				  + uri_text + "\".");
        // scheme can be missing if this is a relative uri
        return scheme_start;
    }
					    ...
					    std::string::const_iterator content_start)
  {
	...
	authority_cursor = (m_content.begin() + 2);
	...
	if (*authority_cursor == ':')
	{
	  authority_cursor = parse_port(uri_text, m_content, (authority_cursor + 1));
          ...
          if (authority_cursor == path_end)
          {
              // Then there is no path.
              path_start = authority_cursor;
          }
      }
