        ...
        : m_tot_len(0), m_len(0)
{
    ...
    m_h[6] = 0x1f83d9ab;
    m_h[7] = 0x5be0cd19;
    memset(m_block, 0, 2*SHA224_256_BLOCK_SIZE);
}
...
void SHA256::transform(const unsigned char *message, unsigned int block_nb)
{
    ...
    for (i = 0; i < block_nb; i++) {
        ...
        for (j = 0; j < 16; j++) {
            SHA2_PACK32(&sub_block[j << 2u], &w[j]);
        }
        for (j = 16; j < 64; j++) {
...
std::string SHA256::hexEncode(const unsigned char *digest) {
...
std::string SHA256::digest() {
    // assumes init() has been called and some data has been given via update()
...
std::string sha256(const std::vector<std::string> &input) {
    ...
    for(const auto& s : input) {
        ctx.update( (const unsigned char*)s.c_str(), s.length());
    }
    ctx.final(digest);
    return ctx.hexEncode(digest);
}
